...
def paginate():
    ...
    if not os.path.exists('output_files/' + code + '.txt'):
        return "please enter the valid token"
    with open("number_of_lines.txt") as f:
        ans = ""
        for i in f:
            ans = ans + i
        dict = json.loads(ans)
    total_number_of_lines = dict[code]
    if total_number_of_lines % per_page == 0:
        total_number_of_pages = (total_number_of_lines/per_page) - 1
    else:
        total_number_of_pages = math.floor(total_number_of_lines/per_page)
...
def file_comp():
    ...
    if file.total_threads >= 2:
        return "2 request are already processing"
